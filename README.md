...
```
小脑服务（/etc/systemd/system/start-cerebellum.service）
大脑服务（/etc/systemd/system/start-brain.service）
#查看所有话题
docker exec brain bash -c "source /ros2_ws/install/setup.bash && ros2 topic list"

开启自启动服务
sudo systemctl start start-cerebellum.service
sudo systemctl start start-brain.service
开启自启动服务
sudo systemctl enable start-cerebellum.service
sudo systemctl enable start-brain.service

停止自启动服务
sudo systemctl stop start-cerebellum.service
sudo systemctl stop start-brain.service
关闭自启动服务
sudo systemctl disable start-cerebellum.service
sudo systemctl disable start-brain.service
查看状态
sudo systemctl status start-cerebellum.service
sudo systemctl status start-brain.service
...
- rosbridge.launch.py：基于WebSocket的ROS2消息桥接服务，使得外部网络可以发现机器的ROS2话题并进行通信，端口9090用于大消息的传输，9091用于控制流的传输

- env.sh：开机自启动的环境设置

- monitor_rosbridge.py：解决rosbridge无连接时积压内存的问题，来源于rosbridge本身的bug，连接时上层会进行数据处理并且清理逻辑正常，服务端不积压因此不会出现问题。无连接时还是会处理ros消息，但是清理逻辑被挂起导致内存泄露。该脚本用于监控rosbridge进程，大于500MB时自动清理并重启进程。

  无连接时由系统清理（无人察觉），有连接时rosbridge原生处理（无人察觉）。

  - **服务调用 (`Service Call`) 未释放**：这是已被官方确认并修复的泄漏点。`rosbridge` 在处理完服务调用后，未能正确销毁相关的客户端对象。每次调用都会残留一些内存，累积起来就造成了泄漏。
  - **客户端断开连接后，清理逻辑挂起**：官方 Issue #891 描述的现象和你遇到的情况几乎完全吻合。在特定条件下（如客户端快速重连、系统负载较高时），`rosbridge` 清理已断开客户端资源的逻辑会挂起。这导致为该客户端分配的内存（包括其传出队列）无法释放，这些“僵尸”资源会持续存在，并可能继续接收并缓存 ROS 网络中的消息，导致内存不断增长。
  - **二进制帧处理缺陷导致处理极慢**：这是一个严重的性能问题。当客户端（如 `ros-sharp`）以二进制帧发送数据时，`rosbridge` 的处理效率会急剧下降。这会拖慢整个 `IncomingQueue` 的处理速度，导致消息积压。虽然这主要影响有连接时的性能，但其造成的处理瓶颈也可能间接影响到无连接时的内存回收效率。
...
#### 18.rosbridge无连接时会内存泄露。

连接时上层会进行数据处理并且rosbridge清理逻辑正常，服务端不积压因此不会出现问题。无连接时还是会处理ros消息，但是清理逻辑被挂起导致内存泄露。monitor_rosbridge.py脚本用于监控rosbridge进程，大于500MB时自动清理并重启进程。
