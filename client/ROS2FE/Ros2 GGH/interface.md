...
> **版本**：v1.0 ｜ **更新日期**：2026-07-26 ｜ **rosbridge 地址**：`ws://192.168.1.30:9090`
...
- [代价地图（订阅）](#代价地图订阅)
- [调试与可视化（订阅）](#调试与可视化订阅)
- [日志与系统状态（订阅）](#日志与系统状态订阅)
- [生命周期事件（订阅，调试用）](#生命周期事件订阅调试用)
- [其他话题](#其他话题)
- [导航控制（服务/动作）](#导航控制服务动作)
- [话题分类速查表](#话题分类速查表)
- [代码示例](#代码示例)
  - [Python 脚本控制示例](#python-脚本控制示例)
  - [C# 连接示例（RosSharp）](#c-连接示例rossharp)
- [重要说明](#重要说明)
- [附录：完整话题列表](#附录完整话题列表)
...
> ⚠️ 这些话题主要用于调试生命周期节点状态变化，正常使用中可忽略。
...
所有消息类型采用 ROS 2 标准格式 `包名/msg/类型名`，如 `geometry_msgs/msg/Twist`。`rosbridge` 协议中 `type` 字段需要完整填写此格式。

### 2. `rosbridge` 协议中的 `type` 字段

订阅时建议添加 `type` 字段以确保正确解析：
...
`/chassis/motor_cmd` 和 `/chassis/motor_states` 使用的是自定义消息类型，定义在 `interfaces` 包中，需要在两端（大脑/小脑）编译。

### 4. `lifecycle_msgs`

生命周期事件话题主要用于调试，正常使用中可忽略。

---

## 附录：完整话题列表

> 通过 `docker exec brain bash -c "source /ros2_ws/install/setup.bash && ros2 topic list"` 获取的真实话题列表：

```bash
gh@ggh-desktop:~$ docker exec brain bash -c \
  "source /ros2_ws/install/setup.bash && ros2 topic list"
```

<details>
<summary>📋 展开查看完整话题列表（共 54 项）</summary>
...
/cmd_vel_safe
/connected_clients
/controller_manager/transition_event
