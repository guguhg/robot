...
public:
        ...
        : Node("imu_tools_node")
    {
        ...
        RCLCPP_INFO(this->get_logger(), "  zero_point: %s", config_.zero_point_enabled ? "enabled" : "disabled");
        RCLCPP_INFO(this->get_logger(), "  publish_relative: %s", config_.publish_relative ? "true" : "false");
    }
...
private:
    ...
    void loadConfig()
    {
        try
        {
            ...
            if (imu_tools_config)
            {
                ...
                config_.stationary_samples = std::clamp(config_.stationary_samples, 10, 500);
    ...
    void onIMU(const sensor_msgs::msg::Imu::SharedPtr msg)
    {
        ...
        // ---------- 发布四元数 ----------
        // 根据配置选择发布相对四元数还是绝对四元数
        // 相对四元数：零点校准后，静止时为 (1,0,0,0)
        // 绝对四元数：原始姿态，包含 IMU 安装倾斜
