...
{
    switch (level) {
        case LogLevel::DEBUG: return LOG_COLOR_GRAY;
        case LogLevel::INFO:  return LOG_COLOR_GREEN;
        case LogLevel::WARN:  return LOG_COLOR_YELLOW;
        case LogLevel::ERROR: return LOG_COLOR_RED;
        case LogLevel::FATAL: return LOG_COLOR_RED;
    ...
    , file_initialized_(false)
{
    ...
    // 读取级别（数字）
    int level_num = logger_config["level"].as<int>(1);
    setLevel(static_cast<LogLevel>(level_num));
    
    // 读取控制台输出
    bool console = logger_config["console"].as<bool>(true);
    setConsoleOutput(console);
    
    // 读取文件路径
    std::string file_path = logger_config["file"].as<std::string>("");
